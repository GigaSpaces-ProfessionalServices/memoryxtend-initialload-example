...
public class TraceHelper {
    ...
    static String zipkinLookupGroups = "";
    static String zipkinUrl = "";

    private static ZipkinTracerBean tracingBean;
    private static TraceHelper helper;
    public static String zipkinurl = zipkinUrl;


    protected TraceHelper() {
        tracingBean = new ZipkinTracerBean(zipkinTracerBean).setStartActive(true).setZipkinUrl(zipkinUrl);
        tracingBean.start();
    }
    ...
    public static void main(String[] args) throws Exception {
        spaceName = args[0];
        zipkinTracerBean = args[1];
        zipkinUrl = args[2];
        zipkinLookupGroups = args[3];

        TraceHelper helper = TraceHelper.getInstance();
        Tracer tracer = GlobalTracer.get();
        gs = new GigaSpaceConfigurer(new SpaceProxyConfigurer(spaceName).lookupGroups(zipkinLookupGroups)).create();
        wrap("write", () -> {
            write();
            return 0;
        });
